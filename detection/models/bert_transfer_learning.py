...
if is_tf_with_gpu:
    ...
    if len(devices) > 0:  # gpu available
        ...
        LSTM_LAYER = LSTM
from transformers import AutoTokenizer, DistilBertTokenizer, TFBertModel, TFRobertaModel, TFXLNetModel, TFDistilBertModel, TFBertEmbeddings
...
class BertFineTuning(NeuralNetworkModel):
    ...
    def build_model(self):
        ...
        """
        print("building model with vocab size", self.vocab_size)
        # pre-trained bert with config
        tokenizer = AutoTokenizer.from_pretrained(self.bert_conf['model_type'])
        ...
        # inputs
        idx = Input((max(self.max_seq_len, self.vocab_size)), dtype="int32", name="input_idx")
        if self.bert_conf['use_masking']:
            masks = Input((max(self.max_seq_len, self.vocab_size)), dtype="int32", name="input_masks")
            bert_out = transformer_model([tokenizer(idx), tokenizer(masks)])[0]

        else:
            bert_out = transformer_model(tokenizer(idx))[0]
