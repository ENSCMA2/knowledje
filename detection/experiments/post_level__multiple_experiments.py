...
def run_multiple_plm_experiments():
    ...
    for model_type in ["lr", "feed_forward_nn", "attenion_lstm"]:# , "lightgbm", "catboost", "xgboost", "feed_forward_nn", "cnn_lstm", "attenion_lstm"]:#, "bert_transfer_learning_without_finetuning", "bert_transfer_learning_with_finetuning"]:
    # for model_type in ["bert_transfer_learning_without_finetuning", "bert_transfer_learning_with_finetuning"]:
        ...
        if not train_on_all_data and X_test is not None:
            ...
            result.to_csv(os.path.join(f"detection/outputs/{dataset_name}", f"post_level_results__{int(test_size * 100)}_test_size.tsv"), sep='\t', index=False)

            errors = [["X_test", "y_test", "y_pred"]]
            for i in range(len(y_pred)):
                errors.append([X_test_as_text.iloc[i], y_test.iloc[i], y_pred[i]])
            error_path = os.path.join(f"detection/outputs/{dataset_name}", f"post_level_errors__{model_type}__{int(test_size * 100)}_test_size.tsv")
            with open(error_path, "w") as o:
